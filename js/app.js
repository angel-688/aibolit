    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".slider-main__body")) new core(".slider-main__body", {
                ...
                on: {
                    init: function() {
                        const pagination = document.querySelectorAll(".main-slider__pagination .swiper-pagination-bullet");
                        pagination.forEach(((el, index) => {
                            el.insertAdjacentHTML("afterbegin", `<span>${index + 1}</span>`);
                            let createSpanRight = document.createElement("div");
                            let createSpanLeft = document.createElement("div");
                            createSpanLeft.classList.add("left");
                            createSpanRight.classList.add("right");
                            el.prepend(createSpanLeft, createSpanRight);
                        }));
                    }
                }
            });
        ...
        document.addEventListener("click", (function(e) {
            const target = e.target;
            if (isMobile.any()) if (target.classList.contains("menu__arrow")) target.closest(".menu__item").classList.toggle("_hover");
            if (target.classList.contains("popup__item")) {
                const popupItemValue = target.textContent;
                document.querySelector(".header__city-button").textContent = popupItemValue;
                modules_flsModules.popup.close("#popup");
            }
        }));
        function initMap() {
            const location = [ {
                position: new google.maps.LatLng(48.764504, 44.478776)
            }, {
                position: new google.maps.LatLng(48.679241, 44.472357)
            } ];
            const options = {
                center: new google.maps.LatLng(48.764504, 44.478776),
                zoom: 11,
            ...
            const map = new google.maps.Map(document.querySelector(".contacts__map"), options);
            ...
            function MapZoomControl(controlDiv, map) {
                ...
                controlUI.className = "customZoom";
                controlDiv.appendChild(controlUI);
                let controlZoomIn = document.createElement("button");
                controlZoomIn.className = "customZoomIn";
                controlUI.appendChild(controlZoomIn);
                let controlZoomOut = document.createElement("button");
                controlZoomOut.className = "customZoomOut";
            ...
            new MapZoomControl(controlDiv, map);
